...
import { Manager } from "@/app/components/Table/types/table";
import { API_ENDPOINTS } from "../constants/apiEndpoints";

export const managerService = {
  async getManagersPermissions(projectId: number): Promise<Manager[]> {
    const { data } = await apiClient.get<Manager[]>(API_ENDPOINTS.MANAGER.PERMISSIONS(projectId));
    return data;
  },
  ...
  async updateManagerPermission(
    ...
    await apiClient.patch(API_ENDPOINTS.MANAGER.UPDATE_PERMISSION(permissionId), {
      project: projectId,
      user: userId,
      ...permissions
    });
