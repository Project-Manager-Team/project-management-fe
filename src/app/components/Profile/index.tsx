...
const Profile = () => {
    ...
    image.onload = async () => {
      ...
      canvas.toBlob(async (blob) => {
        if (blob) {
          ...
          try {
            const response = await apiClient.post("/api/user/profile/update-avatar/", formData, {
  ...
  if (!user) return (
    <div className="relative profile-avatar">
      <AvatarDisplay onClick={() => setIsVisible(prev => ({ ...prev, showLogin: !prev.showLogin }))} />
      {isVisible.showLogin && (
  ...
  return (
    <div className="relative profile-avatar">
      <AvatarDisplay 
        avatar={user.avatar}
        onClick={() => setIsVisible(prev => ({ ...prev, showInfo: !prev.showInfo }))}
      />
      <input
        type="file"
        accept="image/*"
      ...
      {isVisible.showInfo && !isVisible.changePass && (
        <ProfileDropdown 
          user={user}
          onChangePassword={() => setIsVisible(prev => ({ ...prev, changePass: true }))}
      ...
      {isVisible.changePass && (
        <ChangePasswordForm 
          ...
          onBack={() => setIsVisible(prev => ({ ...prev, changePass: false }))}
        />
      )}
    </div>
  );
